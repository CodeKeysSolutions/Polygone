    ...
    for k, v in pairs(FW.Functions.GetPlayers()) do
        ...
        if Player then
            table.insert(Players, {
                ...
                GradeName = FW.Shared.Jobs[Player.PlayerData.job.name].grades[Player.PlayerData.job.grade.level] and FW.Shared.Jobs[Player.PlayerData.job.name].grades[Player.PlayerData.job.grade.level].name or "Invalid grade",
...
AddEventHandler('Admin:Revive:In:Distance', function()
    if not IsPlayerAdmin(source) then print(source .. " tried to execute an admin option, but is not an admin... Cheaterrrrrr") return end
...
AddEventHandler('fw-admin:Server:Request:Job', function(Target, Job, Grade)
    ...
    if TPlayer ~= nil then
        ...
        TriggerClientEvent('FW:Notify', source, GetPlayerName(Target) .. " their job changed to: " .. Job .. " (Grade: " .. Grade .. ")", "success")
    end
end)
...
AddEventHandler('Admin:RequestMoney', function(Result)
    ...
    if Result.action == 'give' then
        ...
        TriggerClientEvent('FW:Notify', source, GetPlayerName(Result.player) .. " receives $" .. Result.amount .. " " .. Result.type .. '!', "success")
    else
        TPlayer.Functions.SetMoney(Result.type, tonumber(Result.amount))
        TriggerClientEvent('FW:Notify', source, "Money of " .. GetPlayerName(Result.player) .. " set to $" .. Result.amount .. " " .. Result.type .. '!', "success")
    end
end)
...
AddEventHandler('Admin:Wipe:Inventory', function(Result)
    ...
    Player.Functions.Notify("Inventory of " .. GetPlayerName(Result.player) .. " has been wiped.", "success")
end)
...
AddEventHandler('Admin:Spawn:Item', function(Result)
    ...
    TriggerClientEvent('FW:Notify', source, "You gave " .. Amount .. 'x ' .. ItemName .. ' (' .. Result.customType .. ') to ' .. GetPlayerName(Result.player) .. "!", "success")
...
AddEventHandler('Admin:Remove:Item', function(Result)
    ...
    if TPlayer.Functions.RemoveItemByName(Result.item, tonumber(Result.amount) or 1, true) then
        TriggerClientEvent('FW:Notify', source, "You removed " .. Result.amount .. 'x ' .. Result.item .. ' from ' .. GetPlayerName(Result.player) .. "!", "success")
    else
...
AddEventHandler('Admin:Set:High:Command', function(Result)
    ...
    if Result.state:lower() == 'true' then
        ...
        TriggerClientEvent('FW:Notify', source, 'Player is now a high command!', 'success')
        ...
        TriggerClientEvent('FW:Notify', TPlayer.PlayerData.source, 'You are no longer a high command!', 'error')
        TriggerClientEvent('FW:Notify', source, 'Player is NO LONGER a high command!', 'error')
...
AddEventHandler('Admin:Request:Gang', function(Result)
    ...
    TriggerClientEvent('FW:Notify', source, "Gang set from " .. GetPlayerName(Result.player) .. " to " .. Result.gang .. "!", "success")
end)
...
AddEventHandler("Admin:Announce", function(Result)
    ...
    TriggerClientEvent('chatMessage', -1, "Announcement", "error", Result.msg)
end)
...
AddEventHandler("Admin:Kick:Player", function(Result)
    ...
    DropPlayer(TPlayer.PlayerData.source, 'You were kicked from the server, reason:\n' .. Result.reason .. '\n\n🔸 If you have questions, create a ticket in our discord: https://discord.gg/gGYt5mbrTW')
    TriggerClientEvent('FW:Notify', source, GetPlayerName(Result.player) .. " has been kicked for " .. Result.reason .. "!", "success")
end)
...
AddEventHandler("Admin:Ban:Player", function(Result)
    ...
    TriggerEvent('fw-logs:Server:Log', 'bans', 'Player Banned', GetPlayerName(TPlayer.PlayerData.source)..' was banned by '..GetPlayerName(source)..' for reason '..Result.reason, 'red')

    if ExpireTime >= 3132036000 then
        DropPlayer(TPlayer.PlayerData.source, 'You were banned for the reason:\n' .. Result.reason .. '\n\nYour ban is permanent.\n🔸 If possible, you can still create a ticket in our discord: https://discord.gg/gGYt5mbrTW')
    else
        DropPlayer(TPlayer.PlayerData.source, 'You were banned for the reason:\n' .. Result.reason .. '\n\nBan expires on: ' .. TimeTable['day'] .. '/' .. TimeTable['month'] .. '/' .. TimeTable['year'] .. ' ' .. TimeTable['hour'] .. ':' .. TimeTable['min'] .. '\n🔸 If possible, you can still create a ticket in our discord: https://discord.gg/gGYt5mbrTW')
    end
end)
...
AddEventHandler('Admin:Server:SetGangOwner', function(Result)
    ...
    if exports['fw-laptop']:SetGangLeader(Result.Gang, TPlayer.PlayerData.citizenid) then
        Player.Functions.Notify("Leadership of " .. Result.Gang .. " set to " .. TPlayer.PlayerData.citizenid, "success")
    end
end)
...
AddEventHandler('Admin:Server:GiveStarterCar', function(Result)
    ...
    Player.Functions.Notify("You have given away a starter car!", "success")
end)
...
FW.Commands.Add("report", "Send a report. Player Reports and Refunds go via a ticket. Only use when really necessary.", {{name="bericht", help="Message you want to send"}}, true, function(source, args)
    local src = source
    local msg = table.concat(args, ' ')
    local Player = FW.Functions.GetPlayer(source)
    TriggerClientEvent('FW:Notify', source, "Your report has been received. If you do not get a response, please create a ticket!", "success")
...
FW.Commands.Add("reportr", "Reply to a report", {}, false, function(source, args)
    ...
    if OtherPlayer ~= nil then
        ...
        TriggerClientEvent('FW:Notify', source, "Person is not online", "error")
    end
end, "admin")
...
FW.Commands.Add('reporttoggle', 'Toggle Incoming Reports (Admin Only)', {}, false, function(source, args)
    ...
    if FW.Functions.IsOptin(src) then
        TriggerClientEvent('FW:Notify', source, "You WILL receive reports", "success")
    else
        TriggerClientEvent('FW:Notify', source, "You will NOT receive reports", "error")
    end
end, 'admin')
...
AddEventHandler("txAdmin:events:announcement", function(eventData)
    for i = 1, 3, 1 do
        TriggerClientEvent('chatMessage', -1, "Announcement", "error", eventData.message)
    end
end)
