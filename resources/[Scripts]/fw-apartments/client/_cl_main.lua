...
Citizen.CreateThread(function()
    exports['PolyZone']:CreateBox({ center = vector3(-271.15, -957.91, 31.22), length = 2.2, width = 1.4 }, {
        ...
        SetInteraction('[E] Enter partment, [G] More options')
                ...
                if IsControlJustReleased(0, 47) then
                    ...
                    local ContextItems = {
                        {
                            ...
                            Desc = 'lock/unlock you apartment.',
                    ...
                    if PlayerData.job.name == "police" or PlayerData.job.name == "judge" then
                        ...
                        ContextItems[#ContextItems + 1] = {
                            Icon = 'door-closed',
                            Title = 'Lockdown Apartments',
                            Desc = 'View / Enter apartments who are lockeddown.',
                            Data = { Event = '', Type = 'Client' },
...
RegisterNetEvent('fw-apartments:Client:EnterApartment', function(Data)
    ...
    if exports['fw-cityhall']:IsLockdownActive("apartments-" .. Data.RoomId) and PlayerData.job.name ~= 'police' and PlayerData.job.name ~= 'judge' then
        FW.Functions.Notify("Cant do this at this time..", "error")
    else
...
function EnterApartment(RoomId, WakeUp)
    ...
    if InteriorData == nil or InteriorData[1] == nil then return FW.Functions.Notify("Cant load apartment..", "error") end
    ...
    if WakeUp then
        print("Waking up in apartment: " .. CurrentRoomId)
        -- Sofa
        ...
        print("Requesting anim dict: switch@franklin@bed")
        local timeout = 0;
        while timeout < 5 and not HasAnimDictLoaded("switch@franklin@bed") do Citizen.Wait(1000) timeout = timeout + 1 end

        Citizen.Wait(1000)
        print("Starting animation: sleep_getup_rubeyes")
            ...
            if NearAnything then
                if not ShowingInteraction then
                    if InteractType == "Exit" then
                        SetInteraction("[E] Leave", "primary") -- was "[E] Verlaten"
                    end
                    if InteractType == "Storage" then
                        SetInteraction("[E] Storage", "primary") -- was "[E] Opslag"
                    end
                    if InteractType == "Closet" then
                        SetInteraction("[E] Wardrobe", "primary") -- was "[E] Kledingkast"
                    end
                    if InteractType == "Logout" then
                        SetInteraction("[E] Sleep", "primary") -- was "[E] Slapen"
                    end
                end
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    if InteractType ~= "Exit" then
                        ...
                        if IsLockdownActive and PlayerData.job.name ~= 'police' and PlayerData.job.name ~= 'judge' then
                            FW.Functions.Notify("You cannot do this right now..", "error") -- was "Cant do this right now.."
                            goto Skip
                        end
                    end
