...
onNet("fw-businesses:Client:AutoCare:CheckVehicle", async (Data: any, Entity: number) => {
    ...
    if (!VehicleParts[Plate]) {
        return FW.Functions.Notify("You see few screws and are surprised this vehicle even moves..", "error")
    };

    const VehicleData = FW.Shared.HashVehicles[GetEntityModel(Entity)]
    if (!VehicleData) {
        return FW.Functions.Notify("This vehicle cannot be checked..", "error")
    };
    ...
    ContextItems.push({
        ...
        Desc: "View the state of the parts.",
        Data: { Event: '', Type: 'Client'},
        SecondMenu: []
    });
    ...
    if (await IsPlayerInBusiness("Bennys Motorworks") && !exp['fw-vehicles'].IsGovVehicle(Entity)) {
        ContextItems.push({
            ...
            Desc: "Install a racing harness or nitrous bottle.",
            Data: { Event: '', Type: 'Client'},
            SecondMenu: [
                {
                    Icon: 'user-slash',
...
onNet("fw-businesses:Client:AutoCare:MountPart", (Item: string, Type: string, Class: string) => {
    if (IsPedInAnyVehicle(PlayerPedId(), false)) {
        return FW.Functions.Notify("You can't mount a vehicle part from here..", "error")
    };
    ...
    if (VehicleData.Class != Class) {
        return FW.Functions.Notify("It looks like this part doesn't fit this vehicle..", "error")
    }
    ...
    FW.Functions.Progressbar("mounting_part", "Repairing...", 12500, false, true, {
        ...
        if (!DidRemove) {
            return FW.Functions.Notify("Where did the vehicle part go then?", "error")
        };
...
onNet("fw-businesses:Client:AutoCare:MountMisc", (Data: {
    ...
    if (!Item) return FW.Functions.Notify("You're missing an item..", "error");

    if (Data.Misc == "Nitrous" && !IsToggleModOn(Data.Vehicle, 18)) {
        return FW.Functions.Notify("This vehicle doesn't have a Turbo to install the nitrous bottle on..", "error")
    };
...
onNet("fw-businesses:Client:AutoCare:ApplyWax", async () => {
    ...
    const Plate = GetVehicleNumberPlateText(Entity)
    if (!VehicleParts[Plate]) {
        return FW.Functions.Notify("You can't wax this vehicle..")
    }
    
    if (GetVehicleDirtLevel(Entity) > 1.0) {
        return FW.Functions.Notify("The car is dirty.. Maybe clean it first?")
    }

    const WaxPercentage = exp['fw-vehicles'].GetVehicleMeta(Entity, "Waxed")
    if (WaxPercentage && WaxPercentage > 25.0) {
        return FW.Functions.Notify("The car is still waxed.. Try again later!")
    }
