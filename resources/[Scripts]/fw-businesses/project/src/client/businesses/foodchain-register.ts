...
on("fw-businesses:Client:Foodchain:SetupPayment", async (Data: {
    ...
    const Result = await exp['fw-ui'].CreateInput([
        { Label: "Cost", Icon: "fas fa-euro-sign", Name: "Costs", Type: "number" },
        { Label: "Order", Icon: "fas fa-pencil", Name: "Comment" },
    ]);
...
on("fw-businesses:Client:Foodchain:GetPayments", async (Data: {
    ...
    if (!Result) {
        return FW.Functions.Notify("No outstanding order..", "error");
    };
