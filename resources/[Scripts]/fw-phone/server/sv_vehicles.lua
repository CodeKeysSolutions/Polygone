...
FW.Functions.CreateCallback("fw-phone:Server:Vehicles:GetVehicles", function(Source, Cb)
    ...
    for k, v in pairs(PlayerVehicles) do
        ...
        Vehicles[#Vehicles + 1] = {
            ...
            Plate = v.plate,
            State = v.state == 'in' and 'Inside' or v.state == 'depot' and 'Depot' or 'Oudside',
...
AddEventHandler("fw-phone:Server:SellVehicle", function(Data)
    ...
    if #(GetEntityCoords(Vehicle) - GetEntityCoords(GetPlayerPed(Source))) > 5.0 then
        return Player.Functions.Notify("To far from vehicle..", "error")
    end

    -- Do not allow to re-sell Gifted vehicles.
    if exports['fw-vehicles']:GetVehicleMeta(OutsideVehicles[Data.Plate], "Gifted") then
        return Player.Functions.Notify("You cant sell this vehicle to citizen..", "error")
    end

    if #(GetEntityCoords(GetPlayerPed(Target.PlayerData.source)) - GetEntityCoords(GetPlayerPed(Source))) > 5.0 then
        return Player.Functions.Notify("To far..", "error")
    end

    TriggerClientEvent("fw-phone:Client:Notification", Target.PlayerData.source, "purchase-vehicle-" .. Data.Cid, "fas fa-car", { "white", "rgb(38, 50, 56)" }, "Buy vehicle", exports['fw-businesses']:NumberWithCommas(tonumber(Data.Amount)), false, true, "fw-phone:Server:AcceptVehicleOffer", "fw-phone:Client:RemoveNotificationById", { Id = "purchase-vehicle-" .. Data.Cid, Cid = Data.Cid, Seller = Source, Amount = Data.Amount, Plate = Data.Plate })
end)
...
AddEventHandler("fw-phone:Server:AcceptVehicleOffer", function(Data)
    ...
    TriggerClientEvent('fw-phone:Client:UpdateNotification', Source, Data.Id, true, true, false, "Buying...", true)

    Citizen.SetTimeout(1000, function()
        if exports['fw-financials']:RemoveMoneyFromAccount(Seller.PlayerData.citizenid, Seller.PlayerData.charinfo.account, Player.PlayerData.charinfo.account, Data.Amount, "PURCHASE", "Bought vehicle: " .. Data.Plate) then
            TriggerClientEvent('fw-phone:Client:UpdateNotification', Source, Data.Id, true, true, false, "Transaction complete!", true)
            ...
            exports['fw-financials']:AddMoneyToAccount(Player.PlayerData.citizenid, Player.PlayerData.charinfo.account, Seller.PlayerData.charinfo.account, Data.Amount, 'PURCHASE', 'Vehicle sold: ' .. Data.Plate)
            ...
            TriggerClientEvent('fw-phone:Client:UpdateNotification', Source, Data.Id, true, true, false, "Transaction rejected!", true)
        end
    end)
end)